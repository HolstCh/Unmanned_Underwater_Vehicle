...
class View(CTk.CTkFrame):
    def __init__(self, root, master, **kwargs):
        ...
        left_servo_label = CTk.CTkLabel(master=root, text="Left Thruster Angle (°)", fg_color="gray22", anchor="w")
        left_thruster_label = CTk.CTkLabel(master=root, text="Left Thruster RPM", fg_color="gray22", anchor="w")
        right_servo_label = CTk.CTkLabel(master=root, text="Right Thruster Angle (°)", fg_color="gray22", anchor="w")
        right_thruster_label = CTk.CTkLabel(master=root, text="Right Thruster RPM", fg_color="gray22", anchor="w")
        tail_servo_label = CTk.CTkLabel(master=root, text="Tail Thruster Angle (°)", fg_color="gray22", anchor="w")
        tail_thruster_label = CTk.CTkLabel(master=root, text="Tail Thruster RPM", fg_color="gray22", anchor="w")
        ...
        self.left_gripper_state = CTk.StringVar(value="on")
        self.right_gripper_state = CTk.StringVar(value="on")

        # switch widgets
        self.left_gripper_switch = CTk.CTkSwitch(master=root, text="Open Left Gripper", command=self.get_left_gripper,
                                                 variable=self.left_gripper_state, onvalue="on", offvalue="off", switch_width=75)
        self.right_gripper_switch = CTk.CTkSwitch(master=root, text="Open Right Gripper", command=self.get_right_gripper,
                                                  variable=self.right_gripper_state, onvalue="on", offvalue="off", switch_width=75)
        ...
        self.left_gripper_switch.grid(column=0, row=1, padx=15, sticky="N", pady=40)
        self.right_gripper_switch.grid(column=1, row=1, padx=15, sticky="N", pady=40)

        left_servo_label.grid(column=2, row=0, pady=10)
        self.left_servo_slider.grid(column=2, row=1)

        left_thruster_label.grid(column=3, row=0, padx=10)
        self.left_thruster_slider.grid(column=3, row=1, padx=10)

        right_servo_label.grid(column=2, row=4)
        self.right_servo_slider.grid(column=2, row=5)

        right_thruster_label.grid(column=3, row=4, padx=10)
        self.right_thruster_slider.grid(column=3, row=5, padx=10)

        tail_servo_label.grid(column=2, row=6)
        self.tail_servo_slider.grid(column=2, row=7)

        tail_thruster_label.grid(column=3, row=6, padx=10)
        self.tail_thruster_slider.grid(column=3, row=7, padx=10)
